...
public class MutualInformation  {
	...
	private double AcutalTotal =0; 
	
	public MutualInformation(Text sequence,ArrayList<Double> LabelInfor) {
...
*/		
		...
		this.AcutalTotal = LabelInfor.get(2);
		this.entropy =-1* ((pos/posneg)*Math.log((pos+SMALL)/(posneg+SMALL))/log2 + (neg/posneg)*Math.log((neg+SMALL)/(posneg+SMALL))/log2);
	...
	public double ComputeMutual() {		
		...
		this.sequence.clear();
		this.AttValue.clear();
		//return all/this.AcutalTotal*(this.entropy-sum);
		return (this.entropy-sum);
		
	}
	
	public double OptimalMutual() {


		double p1= 0,p2= 0;
		double Limpression=0, Lclick=0, Rimpression=0, Rclick=0;
		int index = 0;
		double IGL, IGR, IG, maxIG= 0;
		double all= this.sequence.get(this.sequence.size()-1);
		if(this.sequence.size()==4) {	
			...
			//maxIG=all/this.AcutalTotal*(this.entropy-IGL);
			maxIG=(this.entropy-IGL);
			...
			for(int i= 0; i< (this.sequence.size()-1)/3-1; i++) {
				...
				//IG= all/this.AcutalTotal*(this.entropy- IGL- IGR);
				IG= (this.entropy- IGL- IGR);
				if( IG >= maxIG) {
					maxIG = IG;
					index = i;
					Limpression = Lall;
